...
namespace MVCForum.Website.Application
{
    public static class AppHelpers
    {
        ...
        {
            return String.Concat("~/Areas/Forum/content/badges/", badgeFile);
        }
        ...
        public static string MemberImage(string avatar, string email, int userId, int size)
        {
            ...
            {
                // Has an avatar image
                return VirtualPathUtility.ToAbsolute(string.Concat(Path.GetDirectoryName(avatar),"/", Path.GetFileNameWithoutExtension(avatar),"-"+size.ToString()+"x" +size.ToString(), Path.GetExtension(avatar)));
            }
            return StringUtils.GetDefaultUserImage(size);
        }
