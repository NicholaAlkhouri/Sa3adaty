...
namespace MVCForum.Website.Controllers
{
    public partial class MembersController : BaseController
    {
        ...
        public ActionResult SrubAndBanUser(int id)
        {
        ...
        [Authorize]
        public ActionResult BanMember(int id)
        {
            using (var unitOfWork = UnitOfWorkManager.NewUnitOfWork())
            {
                var user = MembershipService.GetUser(id);
        ...
        [Authorize]
        public ActionResult UnBanMember(int id)
        {
            using (var unitOfWork = UnitOfWorkManager.NewUnitOfWork())
            {
                var user = MembershipService.GetUser(id);
        ...
        public ActionResult GetByName(string slug)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                ...
                var loggedonId = UserIsAuthenticated ? LoggedOnUser.Id : 0;
                var permissions = RoleService.GetPermissions(null, UsersRole);
        ...
        public ActionResult Edit(int id)
        {
            using (UnitOfWorkManager.NewUnitOfWork())
            {
                var loggedOnUserId = (LoggedOnUser != null ? LoggedOnUser.Id : 0);
