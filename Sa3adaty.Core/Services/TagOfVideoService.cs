...
{
    public class TagOfVideoService
    {
        #region Privates
        ...
        #region Constructor
            public TagOfVideoService(DataAccessManager unit_of_work)
            {
        ...
        #region Methods
            ...
            {
                IQueryable<TagOfVideo> result;

                result = DAManager.TagOfVideoRepository.Get(null, a => (order_dir == "asc" ? a.OrderBy(c => c.TagName) : a.OrderByDescending(c => c.TagName)));
                ...
                List<TagOfVideo> final_res = result.ToList();
                List<ListItemTagViewModel> final_result = new List<ListItemTagViewModel>();
                foreach (TagOfVideo tag in final_res.ToList())
            ...
            {
                var tag = DAManager.TagOfVideoRepository.Get(t => t.TagId == tag_id).FirstOrDefault();
                if (tag != null)
                {
            ...
            {
                TagOfVideo db_tag = new TagOfVideo() { MetaDescription = tag.MetaDescription, TagName = tag.TagName, MetaTitle = tag.MetaTitle, FrontTitle = tag.FrontTitle, FrontDescription = tag.FrontDescription };
            ...
            {
                return DAManager.TagOfVideoRepository.Get().Select(t => t.TagName).ToList();
            }
        #endregion
    }
}
